...
class IamADeveloper(unittest.TestCase):
    ...
    def testLogLevelsAndLog4jConfig(self):
        ...
        insert(app, "conf/log4j.xml", 4, '        <SystemPropertyArbiter propertyName="env" propertyValue="dev">')
        insert(app, "conf/log4j.xml", 5, '            <Console name="Out">')
        insert(app, "conf/log4j.xml", 6, '                <PatternLayout pattern="%m%n"/>')
        insert(app, "conf/log4j.xml", 7, '            </Console>')
        insert(app, "conf/log4j.xml", 8, '        </SystemPropertyArbiter>')
        insert(app, "conf/log4j.xml", 9, '        <SystemPropertyArbiter propertyName="env" propertyValue="prod">')
        insert(app, "conf/log4j.xml", 10, '            <List name="Out">')
        insert(app, "conf/log4j.xml", 11, '            </List>')
        insert(app, "conf/log4j.xml", 12, '        </SystemPropertyArbiter>')
        insert(app, "conf/log4j.xml", 13, '    </Appenders>')
        insert(app, "conf/log4j.xml", 14, '    <Loggers>')
        insert(app, "conf/log4j.xml", 15, '        <Logger name="play" level="debug" >')
        insert(app, "conf/log4j.xml", 16, '            <AppenderRef ref="Out"/>')
        insert(app, "conf/log4j.xml", 17, '        </Logger>')
        insert(app, "conf/log4j.xml", 18, '        <Root level="info">')
        insert(app, "conf/log4j.xml", 19, '            <AppenderRef ref="Out"/>')
        insert(app, "conf/log4j.xml", 20, '        </Root>')
        insert(app, "conf/log4j.xml", 21, '    </Loggers>')
        insert(app, "conf/log4j.xml", 22, '</Configuration>')
